...
class UpdateOrder:
    ...
    def update_order(self,data:KafkaPayload,cancel_ind):
        try:
            test = self.session.query(Order).filter(Order.transaction_id==543).first()
            logger.info(f"by test: {test}")
            logger.info(f"by test: {test.__dict__}")
            ...
            if order_exist is None:
                order_exist = self.session.query(Order).filter(Order.request_id == data.meta.request_id).order_by(Order.transaction_id.desc()).first()
                logger.info(f"by request id: {order_exist}")
                logger.info(f"by request id: {order_exist.__dict__}")
            ...
            if order_exist is None and data.meta.action == TriggerMethod.START_TRANSACTION.value:
                ...
                logger.info(f"latest transaction: {order_exist.__dict__}")
            ...
            if order_exist is None and cancel_ind == True and data.meta.action == TriggerMethod.CANCEL_RESERVATION.value:
            ...
            if order_exist is None and data.meta.action == TriggerMethod.STOP_TRANSACTION.value:
                ...
                logger.info(f"stop_transaction: {order_exist.__dict__}")
            ...
            if order_exist is not None and transaction_exists is not None:
                ...
                if (cancel_ind == True or data.status == 'Rejected') and current_order_status != OrderStatus.CHARGING.value:
